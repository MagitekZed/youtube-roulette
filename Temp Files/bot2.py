...
"""
    ...
    def reroll_superpower(self, call):
        game_session = self.get_game_session(call.message.chat.id)  # Get the game instance for this user
        player_name = call.from_user.first_name  # Get the name of the player
    
        # Check if the reroll superpower has been used
        if game_session.superpowers_used[player_name]["reroll"]:
            self.bot.send_message(call.message.chat.id, "You have already used the reroll superpower in this game.")
        else:
            # Create an instance of InlineKeyboardMarkup
            markup = types.InlineKeyboardMarkup()
